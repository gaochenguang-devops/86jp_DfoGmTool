...
namespace DfoGmTool.Services
{
    ...
    // 与上游的差异: 职业过滤走数值 job + AvatarGrantPolicy.IsUsableByJob(原始 PVF token 串),
    // 不做中文职业名子串匹配 —— ItemEntry.UsableJob 在本项目里必须保持原始 token。
    public sealed partial class PvfIndexService
    {
        ...
        // job 传这个值表示不按职业过滤(预览整套时用)
        internal const int SetAnyJob = -1;
            ...
            out string error)
        {
            ...
            if (items == null)
            {
                error = _buildError != null
                    ? "索引构建失败: " + _buildError
                    : "物品索引还在构建中, 稍等几秒再试";
                return false;
            }

            if (!items.TryGetValue(itemId, out var seed))
            {
                error = "PVF 中没有这件物品";
                return false;
            }
            ...
            var members = ResolveSetMembers(seed, job);
        ...
        internal bool IsSetSendable(ItemEntry entry, int job)
        {
            if (entry == null || entry.PartSetIndex <= 0)
                return false;
            var count = ResolveSetMembers(entry, job).Count;
            return count >= SetSendMinPieces && count <= SetSendMaxPieces;
        }

        private object BuildSetPreview(ItemEntry entry, int job)
        {
            if (entry == null || entry.PartSetIndex <= 0)
                return null;

            var set = GetSet(entry.PartSetIndex);
            var members = ResolveSetMembers(entry, job);
        ...
        private List<ItemEntry> ResolveSetMembers(ItemEntry seed, int job)
        {
        ...
        // 职业适用性与发放校验同源: 直接吃 PVF 原始 usable job 串
        private static bool MatchesUsableJob(ItemEntry item, int job)
        {
            if (job < 0)
                return true;
            ...
            if (string.IsNullOrWhiteSpace(usable))
                return true;
            return AvatarGrantPolicy.IsUsableByJob(usable, job);
        }
