...
{
    // 索引构建时提取预览字段(图标路径/说明文本/属性行), 运行时只读, 不再打开 PVF。
    // 注意: 这里不写 ItemEntry.UsableJob —— 那个字段必须保持 PVF 原始 token 串,
    // 发放校验(AvatarGrantPolicy.IsUsableByJob)和发放页职业筛选都依赖原串;
    // 预览要的中文职业名在 GetItemPreview 里用 UsableJobLabel 现算。
    public sealed partial class PvfIndexService
    {
        ...
        private static readonly Regex ScriptMarkerPattern = new Regex(
            @"\[([^\]/\r\n]+)\]",
            RegexOptions.Compiled);
        // 只用于把说明文本里的 [swordman] 之类标记换成中文, 与 UsableJobLabel 的职业目录无关
        ...
        public object GetItemPreview(int itemId)
        {
            ...
            return new
            {
                ...
                usableJob = UsableJobLabel(entry.UsableJob),
                stats = entry.Stats ?? new List<string>(),
                set = BuildSetPreview(entry, SetAnyJob),
        ...
        private static string SanitizeScriptText(string value)
        {
            ...
            // 先把转义的 %% 藏成占位字符, 免得被下面的 %xxx% 格式串清理规则吃掉
