...
namespace DfoGmTool
{
    // 本机文件选择框: 浏览器拿不到真实磁盘路径, 数据源面板要填绝对路径只能由后端弹窗。
    // 仅本机模式可用(远程访问在 Program.cs 里直接拒绝), 非 Windows 一律回退成手填。
    internal static class NativePathDialog
    {
        // 同一时间只允许一个弹窗: COM 对话框是模态的, 并发调用会互相抢前台窗口。
        ...
        public static object Pick(BrowsePathRequest request)
        {
            ...
            try
            {
                var kind = (request == null ? string.Empty : request.Kind ?? string.Empty).Trim().ToLowerInvariant();
                string title;
                bool folder;
                ...
                string path = null;
                string error = null;
                var currentPath = request == null ? null : request.CurrentPath;
                // IFileOpenDialog 要求 STA; ASP.NET Core 的线程池线程是 MTA, 必须另起线程。
        ...
        {
            return new { success = false, error = string.IsNullOrWhiteSpace(error) ? "无法打开系统选择框。" : error };
        }
        ...
        // 面板里已经填了路径就从那儿开始定位, 免得每次都从"此电脑"翻起
        private static void ApplyInitialLocation(IFileOpenDialog dialog, string currentPath, string defaultName)
        {
            ...
            {
                // 非法路径(含通配符/无效字符)不值得报错, 直接按默认位置打开
                return;
            }
    ...
    {
        // database / pvf / imagepacks
        public string Kind { get; set; }
        // 面板里当前填的路径, 用来决定弹窗的初始目录
