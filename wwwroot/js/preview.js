// ---- 物品预览：名称旁图标 + 悬浮卡片 ----
// 图标/预览都是纯展示: 后端没配 ImagePacks2 时 /api/items/{id}/icon 回 204,
// 这里直接不插 <img>, 列表退化成纯文字。
// 事件只在 initItemPreview() 里挂一次(document 级委托), 由 bindings.js 调用。
...
// 换数据源(runtimeSourceEpoch 变了)就丢掉旧 PVF 的预览数据
...
// 改了 ImagePacks2 路径后原地补图/去图, 不用重新拉整张列表
...
// 列表里的物品名统一走这里: 带 data-item-id 的 host 才有悬停预览
